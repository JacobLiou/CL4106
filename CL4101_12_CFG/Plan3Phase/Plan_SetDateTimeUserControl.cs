...
{
    public partial class Plan_SetDateTimeUserControl : UserControl
    {
        private string XmlFilePath = Application.StartupPath + pwFunction.pwConst.Variable.CONST_WORKPLANPATH;
        private string _NodeName = "Plan_SetDateTime";
        private string _NodeDescription = "设置时钟";
        private bool m_Checked = false;//是否要检
        private string m_Para = "";//项目参数

        public Plan_SetDateTimeUserControl()
        {
            InitializeComponent();
            Init();
        ...
        private void CreateDefaultData(ref XmlNode xml)
        {
            ...
            xml.AppendChild(clsXmlControl.CreateXmlNode(_NodeName
                    , "ID", "ProductsName"
                    , "Name", "项目参数"
                    , "Value", "DLT645_2007|0|0|0||"));//协议|标识编码|长度|小数点|下发参数
